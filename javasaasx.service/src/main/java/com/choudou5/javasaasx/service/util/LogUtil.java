...
public class LogUtil {
    ...
    private static LogOperationDao logDao = SpringContextHolder.getBean(LogOperationDao.class);
    private static Queue<LogOperationPo> logQueue = new ArrayBlockingQueue<LogOperationPo>(500);
    ...
    private static void saveLog(HttpServletRequest request, Object handler, String desc){
        ...
        LogOperationPo log = new LogOperationPo();
        log.setDesc(desc);
        // 获取Controller信息
        if (handler != null){
...
//				异步保存日志
    ...
    public static class SaveLogThread implements Runnable{
        ...
        @Override
        public void run() {
            List<LogOperationPo> entitys = new ArrayList<LogOperationPo>();
            for (int i = 0; i < BATCH; i++) {
                LogOperationPo log = logQueue.poll();
                if(log == null)
                    break;
                entitys.add(log);
            }
